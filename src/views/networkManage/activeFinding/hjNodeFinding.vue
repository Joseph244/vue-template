...
<template>
    <div :class="$style.container">
        ...
        <div :class="$style.bottom">
            <button class="comBtn" @click="prev"><i class="el-icon-d-arrow-left"></i> 发现接入节点</button>
            <button class="comBtn" @click="next">发现传感器 <i class="el-icon-d-arrow-right"></i></button>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        async addWhiteList() {
            if (this.selectedIds.length === 0) {
                this.$message({ message: '清先选择需要加入白名单的汇聚节点！', type: 'warning' });
                return false;
            }
            await api.addHjWhiteList({
        ...
        async findHjList() {
            const res = await api.findHjList({ stationId: this.stationId });
            this.newList = res.newFinding || [];
            this.registeredList = res.registered || [];
            this.$parent.stepInfo[2].num = this.$parent.stepInfo[2].num || this.newList.length + this.registeredList.length;
        }
    },
    watch: {
        type() {
