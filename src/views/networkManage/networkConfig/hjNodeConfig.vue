...
<template>
    <div :class="$style.hjNodeConfig">
        <pageBread title="汇聚节点配置"></pageBread>
        <configBaseInfo :canReboot="false" :baseInfo="baseInfo" :runInfo="runInfo" @runInfoRefresh="runInfoRefresh" @reboot="reboot"></configBaseInfo>
        <h3>配置信息</h3>
        <div :class="$style.configInfo">
            <div :class="$style.left">
                <sendingParams nodeType="NODE" @query="sendingParamsQuery" @setting="settingSendParams"></sendingParams>
                ...
                <div :class="$style.leftTitleRow">
                    ...
                    <span>
                        ...
                        <el-button size="mini" type="success" @click="resetWhiteList('nodeSensorWhiteList')">重置</el-button>
                    </span>
                </div>
...
export default {
    ...
    methods: {
        ...
        async settingSendParams(params) {
            await networkConfigApi.sendingParamsSave({ ...params, nodeIdList: [this.nodeId], nodeType: 'NODE' });
        ...
        async resetWhiteList(type) {
            await networkConfigApi.resetWhiteList({ resetType: type, nodeId: this.nodeId });
        ...
        async reboot() {
            await networkConfigApi.jrNode_reboot({ nodeId: this.nodeId, panelType: 'MCB' });
        ...
        async settingSensorWhiteList(whiteList, sensorType) {
            ...
            await networkConfigApi.sensorWhiteListSet({
                nodeId: this.nodeId,
                whiteList,
                sensorType: _st,
                nodeType: 'NODE'
        ...
        async sendingParamsQuery() {
            let data = await networkConfigApi.sendingParamsQuery({
                nodeIdList: [{ nodeId: this.nodeId }],
                nodeType: 'NODE'
            });
            this.logInfo.push(`通用参数:${JSON.stringify(data)}`);
        },

        async sensorWhiteListQuery() {
            let data = await networkConfigApi.sensorWhiteListQuery({ nodeId: this.nodeId, nodeType: 'NODE' });
            this.logInfo.push(`传感器白名单: 微功率 ${JSON.stringify(data.wglSensorIds)};低功耗 ${JSON.stringify(data.wglSensorIds)}`);
        },
        async runInfoRefresh() {
            this.runInfo = await networkConfigApi.hjNode_statusInfo({ stationId: this.baseInfo.stationId, nodeId: this.nodeId });
    ...
    async created() {
        this.baseInfo = await networkConfigApi.hjNode_basicInfo({ nodeId: this.nodeId });
