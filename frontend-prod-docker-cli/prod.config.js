module.exports = {
    ...
    106: {
        ...
        webDir: '/opt/iotinfoappv2/dist', // 文件上传服务器绝对路径
        dockerName: 'iotmanageappv2', // docker名称
        dockerWebDir: '/app' // 容器中文件存放位置
        // lastCmd: 'pm2 restart app' // 所有动作执行完之后在服务器上执行的自定义命令(不配置即不执行)
    },

    229: {
        name: '229docker环境', // 环境名称
        host: '192.168.78.229', // 服务器ip地址
        port: 22, // ssh连接端口（一般默认22）
        username: 'root', // ssh登录服务器用户名
        password: '123456', // ssh登录服务器密码
        distPath: 'dist', // 当前工程下该文件夹下所有文件上传服务器（前不加斜杠表示当前工程目录下相对路径，如前加斜杠就是本机绝对路径)
        webDir: '/service/iotmanageapp/deploy', // 文件上传服务器绝对路径
        dockerName: 'iotmanageapp', // docker名称
        dockerWebDir: '/app' // 容器中文件存放位置
        // lastCmd: 'pm2 restart app' // 所有动作执行完之后在服务器上执行的自定义命令(不配置即不执行)
    }
};
